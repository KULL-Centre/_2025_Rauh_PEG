...
def simulate(residues,dirname,proteins,composition,temp,ionic,cutoff,boxlength,lambda_peg=0.2):
    residues = residues.set_index('one')
    residues.loc["J",'lambdas'] = lambda_peg
    ...
    md.Trajectory(np.array(pos).reshape(N_beads,3), top, 0,
                  [L,L,L], [90,90,90]).save_pdb(path+'/top.pdb')
    ...
    check_point = F'{path}/restart_{lambda_peg}.chk'

    # Check if the simulation is new or a continuation of a previous one.
    if os.path.isfile(check_point):
        print('Reading check point file')
        simulation.loadCheckpoint(check_point)
        simulation.reporters.append(app.dcdreporter.DCDReporter(F'{path}/t_{lambda_peg}.dcd'.format(name),
                                                                N_save,enforcePeriodicBox=True,
                                                                append=True))
    else:
    ...
    simulation.reporters.append(app.statedatareporter.StateDataReporter(F'{path}/{lambda_peg}.log',100000,
             step=True,speed=True,elapsedTime=True,separator='\t'))

#    simulation.runForClockTime(70*unit.hour,checkpointFile=check_point,checkpointInterval=2*unit.hour)

    print("No. Steps to Run: ",2000*N_save)
    #Run rounds of simulations to allow for extra checkpoints.
    for i in range(4):
        simulation.step(500*N_save)
        simulation.saveCheckpoint(F'{path}/restart_{lambda_peg}.chk')
    


if __name__=="__main__":
    parser = ArgumentParser()
    parser.add_argument('--peg',nargs='+',required=True)
    ...
    parser.add_argument('--lambda_peg',nargs='?',const='', type=float)
    args = parser.parse_args()
    
    
    residues = pd.read_csv('residues.csv').set_index('three',drop=False)
    proteins = pd.read_pickle('peg_sequences.pkl')
    
    composition = pd.DataFrame(index=args.peg,columns=['N','fasta'])
    composition.N = [int(N) for N in args.numbers]
    composition.fasta = [proteins.loc[name].fasta for name in composition.index]
    
    t0 = time.time()
    simulate(residues,args.dirname,proteins,composition,args.temp,args.ionic,args.cutoff,args.boxlength,args.lambda_peg)
